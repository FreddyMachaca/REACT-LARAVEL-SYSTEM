...
const TblcatalogoListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblcatalogo/view/${data.cat_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblcatalogo/edit/${data.cat_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.cat_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function CatIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblcatalogo/view/${data.cat_id}`}> { data.cat_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblcatalogoListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Catalogo"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblcatalogo/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="cat_id" header="Cat Id" body={CatIdTemplate}  ></Column>
                                    <Column  field="cat_tabla" header="Cat Tabla"   ></Column>
                                    <Column  field="cat_secuencial" header="Cat Secuencial"   ></Column>
                                    <Column  field="cat_descripcion" header="Cat Descripcion"   ></Column>
                                    <Column  field="cat_abreviacion" header="Cat Abreviacion"   ></Column>
                                    <Column  field="cat_estado" header="Cat Estado"   ></Column>
                                    <Column  field="cat_id_superior" header="Cat Id Superior"   ></Column>
                                    <Column  field="cat_adicional" header="Cat Adicional"   ></Column>
...
TblcatalogoListPage.defaultProps = {
	primaryKey: 'cat_id',
	pageName: 'tblcatalogo',
	apiPath: 'tblcatalogo/index',
	routeName: 'tblcatalogolist',
...
export default TblcatalogoListPage;
