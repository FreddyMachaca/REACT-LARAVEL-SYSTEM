...
const TblcarrerasListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblcarreras/view/${data.carr_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblcarreras/edit/${data.carr_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.carr_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function CarrIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblcarreras/view/${data.carr_id}`}> { data.carr_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblcarrerasListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Carreras"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblcarreras/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="carr_id" header="Carr Id" body={CarrIdTemplate}  ></Column>
                                    <Column  field="carr_nombre" header="Carr Nombre"   ></Column>
                                    <Column  field="carr_estado" header="Carr Estado"   ></Column>
...
TblcarrerasListPage.defaultProps = {
	primaryKey: 'carr_id',
	pageName: 'tblcarreras',
	apiPath: 'tblcarreras/index',
	routeName: 'tblcarreraslist',
...
export default TblcarrerasListPage;
