...
const TblcategorialicenciasListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblcategorialicencias/view/${data.bt_lic_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblcategorialicencias/edit/${data.bt_lic_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.bt_lic_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function BtLicIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblcategorialicencias/view/${data.bt_lic_id}`}> { data.bt_lic_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblcategorialicenciasListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Categoria Licencias"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblcategorialicencias/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="bt_lic_id" header="Bt Lic Id" body={BtLicIdTemplate}  ></Column>
                                    <Column  field="bt_lic_categoria_nombre" header="Bt Lic Categoria Nombre"   ></Column>
                                    <Column  field="bt_lic_prefijo" header="Bt Lic Prefijo"   ></Column>
                                    <Column  field="bt_estado" header="Bt Estado"   ></Column>
...
TblcategorialicenciasListPage.defaultProps = {
	primaryKey: 'bt_lic_id',
	pageName: 'tblcategorialicencias',
	apiPath: 'tblcategorialicencias/index',
	routeName: 'tblcategorialicenciaslist',
...
export default TblcategorialicenciasListPage;
