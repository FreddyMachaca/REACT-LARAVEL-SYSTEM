...
const TblgradoacademicoListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: "Search",
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblgradoacademico/view/${data.ga_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblgradoacademico/edit/${data.ga_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.ga_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function GaIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblgradoacademico/view/${data.ga_id}`}> { data.ga_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblgradoacademicoListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Grado Academico"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblgradoacademico/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="ga_id" header="Ga Id" body={GaIdTemplate}  ></Column>
                                    <Column  field="ga_nombre" header="Ga Nombre"   ></Column>
                                    <Column  field="ga_estado" header="Ga Estado"   ></Column>
...
TblgradoacademicoListPage.defaultProps = {
	primaryKey: 'ga_id',
	pageName: 'tblgradoacademico',
	apiPath: 'tblgradoacademico/index',
	routeName: 'tblgradoacademicolist',
...
export default TblgradoacademicoListPage;
