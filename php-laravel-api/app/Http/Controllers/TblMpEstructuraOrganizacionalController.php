...
class TblMpEstructuraOrganizacionalController extends Controller
{
     ...
     */
	function add(TblMpEstructuraOrganizacionalAddRequest $request){
		try {
	...
	/**
     * Get organizational structure and category details
     */
	...
	/**
     * Get tree organizational structure
     */
	public function getTreeOrganizationalEstructure($eo_pr_id) {
		$allRecords = TblMpEstructuraOrganizacional::from('tbl_mp_estructura_organizacional as eo')
			...
			->select(
				'eo.eo_id', 'eo.eo_cp_id', 'eo.eo_prog', 'eo.eo_sprog', 
				'eo.eo_proy', 'eo.eo_unidad', 'eo.eo_descripcion', 
				'eo.eo_cod_superior', 'cp.cp_da', 'cp.cp_ue', 'cp.cp_programa',
				'cp.cp_proyecto', 'cp.cp_actividad', 'cp.cp_fuente', 'cp.cp_organismo'
			)
			->get();
