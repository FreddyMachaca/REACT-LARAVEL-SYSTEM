...
const TblrangoListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblrango/view/${data.ra_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblrango/edit/${data.ra_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.ra_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function RaIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblrango/view/${data.ra_id}`}> { data.ra_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblrangoListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Rango"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblrango/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="ra_id" header="Ra Id" body={RaIdTemplate}  ></Column>
                                    <Column  field="ra_minimo" header="Ra Minimo"   ></Column>
                                    <Column  field="ra_maximo" header="Ra Maximo"   ></Column>
                                    <Column  field="ra_valor" header="Ra Valor"   ></Column>
                                    <Column  field="ra_tabla" header="Ra Tabla"   ></Column>
                                    <Column  field="ra_estado" header="Ra Estado"   ></Column>
...
TblrangoListPage.defaultProps = {
	primaryKey: 'ra_id',
	pageName: 'tblrango',
	apiPath: 'tblrango/index',
	routeName: 'tblrangolist',
...
export default TblrangoListPage;
