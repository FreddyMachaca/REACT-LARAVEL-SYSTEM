...
const TblidiomaListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/tblidioma/view/${data.idm_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/tblidioma/edit/${data.idm_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.idm_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function IdmIdTemplate(data){
		if(data){
			return (
				<Link to={`/tblidioma/view/${data.idm_id}`}> { data.idm_id }</Link>
			);
		}
	}
	...
	return (
<main id="TblidiomaListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title="Tbl Idioma"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/tblidioma/add`}>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="idm_id" header="Idm Id" body={IdmIdTemplate}  ></Column>
                                    <Column  field="idm_nombre" header="Idm Nombre"   ></Column>
                                    <Column  field="idm_estado" header="Idm Estado"   ></Column>
...
TblidiomaListPage.defaultProps = {
	primaryKey: 'idm_id',
	pageName: 'tblidioma',
	apiPath: 'tblidioma/index',
	routeName: 'tblidiomalist',
...
export default TblidiomaListPage;
